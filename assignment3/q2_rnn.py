...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        true_length = min(max_length, len(sentence))
        false_length = max(0, max_length-len(sentence))
        mod_sentence = sentence[:true_length] + [zero_vector]*(false_length)
        labels = labels[:true_length] + [zero_label]*(false_length)
        mask = [True]*true_length + [False]*false_length
        ret.append((mod_sentence, labels, mask))
        ### END YOUR CODE ###
    return ret
...
class RNNModel(NERModel):
    ...
    def add_prediction_op(self):
        ...
        state = tf.zeros(shape=(tf.shape(x)[0], self.config.hidden_size), dtype=tf.float32)
        ### END YOUR CODE
        ...
        with tf.variable_scope("RNN"):
            for time_step in range(self.max_length):
                ...
                _, state = cell(x[:, time_step, :], state)
                state_dropout = tf.nn.dropout(state, dropout_rate)
                curr_pred = tf.matmul(state_dropout, U) + b2
